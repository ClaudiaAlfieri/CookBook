 document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateGenerateButton() {
                if (ingredients.length > 0) {
                    ...
                    generateBtn.textContent =
                        `🤖 Gerar Receitas com ${ingredients.length} ingrediente${ingredients.length > 1 ? 's' : ''}`;
                } else {
        ...
        `;
            ...
            async function generateRecipe() {
                ...
                if (!csrfToken) {
                    showModal('❌ Erro', 'Token CSRF não encontrado. Recarregue a página.');
                    return;
                }
                ...
                try {
                    ...
                    if (response.ok && data.success) {
                        ...
                        showModal('❌ Erro', data.error || 'Erro desconhecido ao gerar receita');
                    }
            ...
            function showModal(title, content, isRecipe = false) {
                ...
                const formattedContent = isRecipe ? formatRecipeText(content) : `<p>${content}</p>`;

                modal.innerHTML = `
            <div class="recipe-content">
                <button class="close-recipe" onclick="closeModal()">&times;</button>
                <div class="recipe-text">
                    <h3 style="color: ${isRecipe ? '#87A96B' : '#e74c3c'};">${title}</h3>
                    ${formattedContent}
                    ${!isRecipe ? '<button onclick="closeModal()" style="background: #e74c3c; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin-top: 15px;">Fechar</button>' : ''}
                </div>
            </div>
        `;
